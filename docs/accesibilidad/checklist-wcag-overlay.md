...
Jira: A12.9 / ADACEEN-140. Extension de navegador 0.7.8, rama `feat/cierre-pendientes-jira`.
...
| 4.1.2 Nombre, funcion, valor (A) | Cumple | Dialogo con `role`/`aria-label`; `settingsBtn` con `aria-expanded`/`aria-controls`; metas y cursos con `aria-pressed`; casillas con `label`; controles de aplicar codigo se deshabilitan cuando "Permitir aplicar codigo" esta apagado (`syncCodeApplicationInputsState`). |
| 4.1.3 Mensajes de estado (AA) | Cumple | Zona de respuestas del tutor con `role="region"` + `aria-live="polite"` y `aria-busy` mientras carga (`tutorResponseRegion`); `statusText`, `setupStatusText`, `teacherQuizStatus`, `tutorFeedbackStatus` con `role="status"`; `authError` con `role="alert"`. Solo se reescriben si cambia el texto (`setTextIfChanged`) para no repetir anuncios en cada render. |
