...
|---|---|---|---|---|---|---|---|---|
| `tutor_decision` | tutor | backend | sistema | Cada vez que el motor de politicas decide una intervencion (overlay o VS Code), bloqueada o no. | Trazabilidad de la decision: evento de politica, tipo de intervencion, etapa de ayuda, motivo, latencia y servidor de inferencia que respondio (GPU de Google Cloud o Mac del laboratorio). | `decisionId`, `channel`, `policyEventType`, `interventionType`, `helpStage`, `reasonCode`, `blocked`, `latencyMs`, `errorHash`, `contextHash`, `exerciseHash`, `metadata.worker` | latencia p50/p95, tasa de bloqueo por politica, uso efectivo de intervenciones | no |
...
| `metadata` | jsonb | Datos adicionales del evento, filtrados por lista blanca de claves. | ninguna | Solo claves de la lista blanca (trigger, pageType, applyMode, linesChanged, line, cached, blocked, scope, mode, reason, worker, editorHost, editorUi...). |
