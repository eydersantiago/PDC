...
| 5 | Extensión de navegador 0.7.11 en los navegadores de la prueba (y en las rutas de carga de AGENTS.md), descargada de `/empezar` después del despliegue: si ya había una 0.7.11 cargada, reemplazar la carpeta igual (el código cambió sin cambiar la versión) | Dueño | A15.3 · ADACEEN-124 | Antes de la prueba | Despliegue, extensiones |
...
| 10 | Hallazgo 5a, corregido en el backend: si el estudiante sale en un equipo y pulsa «Abrir mi editor» en otro antes de 7 días, `GET /api/workspaces/status` reenvía el `prepare` porque no queda sesión `tunnel`, y la VM recibe una sesión nueva (`tests/routes/workspace-routes.test.ts` y `tests/integration/acceso-simplificado.test.ts`). Falta confirmarlo en real: P3.2 debe volver sola a «ADACEEN: <nombre>» | Dueño | A15.3 · ADACEEN-124 | Mañana | Prueba, P3.2 |
...
| 43 | Cerrar las cuentas demo en producción (C20, crítico): `seed()` las recrea en cada arranque con las claves del repositorio, y el administrador demo puede ser el único administrador. `npm run cuentas-demo -- --confirmar` desactiva el estudiante y el docente demo y cambia la clave del administrador (se muestra una vez). Necesita la `DATABASE_URL` de producción en `.env` y que el cortafuegos de la base deje entrar desde tu equipo (por verificar) | Dueño | A13.4 · ADACEEN-112 | Antes de la prueba (P7.3) | [Despliegue](../operacion/despliegue.md), cuentas demo |
