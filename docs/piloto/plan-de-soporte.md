...
Extensión de navegador 0.7.12, VS Code 0.0.32. Lo que ve el estudiante está en
...
| Un docente o administrador no puede conectar VS Code con GitHub | S3 | Esas cuentas no se vinculan por GitHub (`staff_requires_code`). | «Abrir en VS Code de este equipo», o «Copiar codigo para VS Code» en `vscode.dev` y «Tengo un código o sesión» (en VS Code 0.0.31, Tengo un código del navegador). |
...
| «todavía no permite conectar VS Code así» | S2 | El backend de producción no tiene esta versión. | Desplegar; mientras tanto, «Tengo un código o sesión» con el ID de sesión del navegador. |
| El código de GitHub (`github.com/login/device`) venció o «ADACEEN dejo de esperar.» | S3 | No se autorizó en unos 12 minutos, o se cerró la pestaña de ADACEEN. | El botón que nombra el aviso («Abrir mi editor», o «Preparar mi editor» la primera vez) y usar el código nuevo. |
...
| Otro navegador vuelve a pedir «Aceptar y continuar», o no ofrece «Abrir mi editor» con el editor ya preparado | S3 | El backend desplegado no trae la privacidad en el servidor (`POST /api/auth/privacy-acceptance` responde 404) o la extensión es anterior a la 0.7.12. Con el túnel, también si la cuenta de GitHub no está conectada en ADACEEN o `GET /api/workspaces/status` no responde `ready`. | Aceptar de nuevo no hace daño. Para el editor, «Preparar mi editor» lo abre igual (sin pedir otra vez el código de GitHub). Revisar el commit desplegado y la versión de la extensión en `/empezar`. |
| Docente: «Iniciar bloque 1» responde que no hay estudiantes o que solo hay 1 | S3 | Desde la 0.7.12 iniciar un bloque asigna los grupos si no los hay, pero solo con 2 estudiantes activos o más. | Revisar «Administracion de usuarios» (estudiantes activos del docente). Con uno solo, «Asignar grupos A y B» y luego «Iniciar bloque 1». |
