...
export function registerRoutes(app: express.Express, database: AppDatabase) {
  ...
  function buildHealthPayload(req?: express.Request) {
    return {
      ...
      queues: env.targetMode === "queue"
        ? {
          jobs: env.jobsQueueName,
  ...
  async function handleStructuredIntervention(req: express.Request, res: express.Response) {
    try {
      ...
      if (env.targetMode === "queue" && req.path === "/github-mentor") {
        const context: GithubMentorContext = {
          ...rawContext,
          codeSnippet: trimText(rawContext.codeSnippet).slice(0, env.maxMentorCodeChars),
          selection: trimText(rawContext.selection).slice(0, env.maxMentorCodeChars),
        };
        const model = trimText(req.body?.model) || undefined;
        const minVramGb = Number.isFinite(Number(req.body?.minVramGb))
          ? Math.max(0, Math.round(Number(req.body?.minVramGb)))
          : undefined;
        ...
        const job = await enqueueLlmJob(database, {
          ...
          maxOutputChars: Number.isFinite(Number(req.body?.maxOutputChars))
            ? Math.max(1, Math.round(Number(req.body?.maxOutputChars)))
            : undefined,
        });

        logInfo("github_mentor_job_enqueued", {
          request_id: res.locals.requestId || null,
          job_id: job.id,
          model: model || null,
          min_vram_gb: minVramGb ?? null,
          max_items: maxItems,
